...
def test_backends_db_clickhouse_to_documents_method_when_statement_has_no_id(
    ...
    with pytest.raises(
        BadFormatException, match="Statement is missing id or timestamp field"
    ):
        next(documents)
    ...
    assert "Statement is missing id or timestamp field" in caplog.records[0].message
...
def test_backends_db_clickhouse_to_documents_method_with_invalid_timestamp(
    ...
    with pytest.raises(
        BadFormatException,
        match="Statement has an invalid 'id' or 'timestamp' field",
    ):
    ...
    assert caplog.records[0].levelname == "WARNING"
    assert (
        "Statement has an invalid 'id' or 'timestamp' field"
