...
class ClickHouseDatabase(BaseDatabase):
    ...
    @staticmethod
    def _validate_and_parse(statement: dict) -> tuple[uuid.UUID, datetime.datetime]:
        """Validate and parse required statement fields."""
        timestamp_parser = isoparser(sep="T")

        try:
            # Validate that the statement id is a UUID
            event_id = uuid.UUID(statement["id"], version=4)

            # Convert to a timestamp object that ClickHouse can accept
            emission_time = timestamp_parser.isoparse(statement["timestamp"])

            # If the emission time didn't include a timezone, set to UTC. This should
            # be configurable?
            if (
                emission_time.tzinfo is None
                or emission_time.tzinfo.utcoffset(emission_time) is None
            ):
                emission_time = emission_time.replace(tzinfo=pytz.utc)

            return event_id, emission_time
        except KeyError as err:
            raise BadFormatException(
                f"Statement is missing id or timestamp field: " f"{statement}"
            ) from err
        except ValueError as err:
            raise BadFormatException(
                f"Statement has an invalid 'id' or 'timestamp' field: " f"{statement}"
            ) from err
    ...
    def to_documents(
        ...
        for line in stream:
            ...
            try:
                event_id, emission_time = ClickHouseDatabase._validate_and_parse(
                    statement
                )

                document = (event_id, emission_time, statement, json.dumps(statement))

                yield document
            except BadFormatException as exc:
                if ignore_errors:
    ...
    def query_statements_by_ids(self, ids: list[str]) -> list:
        ...
        try:
            for chunk_ids in chunk_id_list():
                ...
                # query_context.set_parameter("table_name", self.event_table_name)
